...
The `post_install_script` variable allows you to specify a custom script that will be automatically executed during instance boot on all your EC2 instances (i.e. App, Zookeeper, and EMR nodes). This enables the installation of additional software or running custom setup commands.
### How It Works

- The script path you provide should be relative to your Terraform working directory — i.e., where you run `terraform apply`.
- On instance startup during the initial boot, the script is executed as the root user.

### Important Notes

- The script runs with root privileges, so you can perform system-level operations.
- Logs generated by installed software should not be written to the root disk, which has limited storage. Instead, write logs to a directory under `/mnt/instance-store` to prevent disk full issues.
- The App instances and EMR cluster are automatically replaced when the script is introduced, changed, or removed. However, the Zookeeper instances have to be replaced manually - see [Upgrading the Zookeeper Cluster](#upgrading-the-zookeeper-cluster) for more details.

### Example Use Case: Installing Monitoring Tool
Here’s a simple `logging_install.sh` example that installs htop, a basic system monitoring tool:
...
mkdir -p /mnt/instance-store/monitoring-logs
echo "$(date): htop installed and post-install script executed." > /mnt/instance-store/monitoring-logs/install.log
```
